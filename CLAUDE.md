...
### Approval requests (CEO remote-approval pilot)
A pilot for a future remote-signing portal: secretaries file documents from 13 group companies (UAE + Syria, static list in **`src/lib/approvals-meta.ts`**, independent of `src/data/companies.ts`) and the CEO approves/holds/rejects them from `/admin/approvals`. Statuses: active → approved / rejected / hold; uploading a new document version always resets a request to **active** and clears the old decision (history survives in `approval_events`).
- **Roles:** a new `"approvals"` Permission (secretaries) + a singular **CEO** account (`admin_users.is_ceo`; a partial unique index enforces at most one; only the SuperAdmin can create/edit/delete it; mutually exclusive with `is_admin`). `canDecideApprovals()` = SuperAdmin or CEO — regular admins see approvals but can't decide. The `signature` Section (`/admin/signature`, the CEO signature image — **upload a PNG/JPG or draw it on a canvas pad** in `SignatureForm.tsx`; the drawn signature is serialized to a transparent PNG client-side and posted through the same `signature` file field) is SuperAdmin + CEO only; its `can()` branch must stay **before** the `isAdmin` catch-all. `getCurrentUser()`/`toUser()` must keep propagating `isCeo` or the role silently dies. CEO `landingPath` is `/admin/approvals`.
- **Data:** tables `approval_requests` / `approval_documents` (versioned) / `approval_events` (timeline) / `approval_signature` (single row, id `'ceo'`) — RLS on, no policies, like everything else. Files live in the **private `approvals` bucket** (`requests/{id}/v{n}-…pdf`, signed copies `…-signed-…`, signature under `signature/`), read via 1-hour signed URLs (`approvalsSignedUrl` in **`src/lib/approvals.ts`**, the server-only data layer).
- **Actions** (`(admin)/admin/approvals/approvals-actions.ts`, `signature/signature-actions.ts`): create / uploadVersion / decide / delete (SuperAdmin-only) / saveSignature. Mutations use optimistic `.eq("current_version"/"status", …)` guards against races and clean up storage objects on partial failure. PDFs are validated at **4 MB** (Vercel's ~4.5 MB body cap; `next.config.ts` sets `serverActions.bodySizeLimit: "6mb"`).
- **Signing:** on approve, **`src/lib/sign-pdf.ts`** stamps the CEO signature into the PDF. The CEO can **place it himself** in a browser editor (`requests/[id]/SignaturePlacer.tsx`, a `pdfjs-dist` page renderer with a draggable/resizable signature box locked to the signature's aspect ratio; dynamic-imported, `ssr:false`) — the chosen page + normalized box flow through hidden `manual_*` form fields to `stampSignature({ manual })`, which stamps that exact spot with no date label. Otherwise placement is auto-detected: (1) a text-layer keyword scan via `unpdf` (EN/AR/TR signature labels + drawn lines, last page preferred), (2) a Claude-vision fallback (env-gated on `ANTHROPIC_API_KEY`, like `translate.ts` — ⚠️ **be aware: with the key set, scanned/label-less PDFs have their last 4 pages sent to the Anthropic API** for placement detection; these are confidential business documents, so the owner should consciously accept that before setting the key in production), (3) fixed bottom-right of the last page. The approval record snapshots the signature image path (`approval_requests.signature_path`) and old signature files are never deleted, so replacing the signature doesn't rewrite history. It **returns `null` on any failure and must never throw** — approval then proceeds record-only (the request page always shows the signature on the approval record). Helvetica can't render Arabic, so the stamp text stays ASCII. `scripts/make-test-pdfs.mjs` + `scripts/test-stamp.mjs` are a dev harness for it (plain `node`, uses Node's TS type-stripping).
